...
from src.backend_capabilities import get_backend_capabilities, CapabilityLevel, UnsupportedOp
...
class BackendReport:
    def __init__(self, backend_name: str, warnings: list[str], unsupported_nodes: int, generated_lines: int):
...
class QiskitBackend:
    ...
    def transpile(self, graph: EQIRGraph, ast: ProgramNode | None = None) -> tuple[str, BackendReport]:
        ...
        if ast.imports and self.capabilities.supports_imports == CapabilityLevel.PARTIAL:
            diag_engine.emit(
                DiagnosticSeverity.WARNING,
                "Imports are only partially supported in Qiskit backend (inlining is simulated).",
                SourceLocation("ast", 1, 0)
            )
        elif ast.imports and self.capabilities.supports_imports == CapabilityLevel.NONE:
            diag_engine.emit(
        ...
        def visit(node: ASTNode):
            ...
            if isinstance(node, FuncDeclNode):
                if self.capabilities.supports_classical_functions == CapabilityLevel.NONE:
                    diag_engine.emit(
                        DiagnosticSeverity.WARNING,
                ...
                if self.capabilities.supports_structs == CapabilityLevel.NONE:
                    diag_engine.emit(
                        DiagnosticSeverity.WARNING,
                ...
                if self.capabilities.supports_loops == CapabilityLevel.NONE:
                    diag_engine.emit(
                        DiagnosticSeverity.WARNING,
                ...
                if self.capabilities.supports_exceptions == CapabilityLevel.NONE:
                    diag_engine.emit(
                        DiagnosticSeverity.WARNING,
                ...
                if self.capabilities.supports_arrays == CapabilityLevel.NONE:
                    diag_engine.emit(
                        DiagnosticSeverity.WARNING,
                ...
                elif self.capabilities.supports_arrays == CapabilityLevel.PARTIAL:
                    diag_engine.emit(
                        DiagnosticSeverity.WARNING,
                ...
                if self.capabilities.supports_field_access == CapabilityLevel.NONE:
                    diag_engine.emit(
                        DiagnosticSeverity.WARNING,
                ...
                if self.capabilities.supports_index_access == CapabilityLevel.NONE:
                    diag_engine.emit(
                        DiagnosticSeverity.WARNING,
