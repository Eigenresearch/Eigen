...
def _generate_html_dashboard(results: dict, bench_dir: str):
    ...
    for name, t in results.items():
        ...
        bars_html += f'<div class="bar-row"><span class="bar-label">{name}</span><div class="bar-bg"><div class="bar-fill" style="width:{pct:.1f}%;background:{color}"></div></div><span class="bar-val">{t:.2f}ms</span></div>\n'
...
<body>
<h1>Eigen 2.7 Meridian — Benchmark Dashboard</h1>
<div class="summary">
...
<div class="footer">Generated by Eigen 2.7 Meridian — {time.strftime('%Y-%m-%d %H:%M:%S')}</div>
</body>
</html>"""
